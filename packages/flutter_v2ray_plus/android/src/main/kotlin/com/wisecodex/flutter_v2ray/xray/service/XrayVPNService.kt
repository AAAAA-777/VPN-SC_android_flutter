...
class XrayVPNService : VpnService() {
    ...
    private fun buildTun2socksCommand(
        ...
        val proxy = if (config.LOCAL_SOCKS5_USER.isNotEmpty() && config.LOCAL_SOCKS5_PASS.isNotEmpty()) {
            val user = java.net.URLEncoder.encode(config.LOCAL_SOCKS5_USER, "UTF-8")
            val pass = java.net.URLEncoder.encode(config.LOCAL_SOCKS5_PASS, "UTF-8")
            "socks5://$user:$pass@127.0.0.1:${config.LOCAL_SOCKS5_PORT}"
        } else {
